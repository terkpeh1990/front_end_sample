...
<template>
  <VDialog max-width="600" :model-value="props.isLocalDialogVisible" :height="$vuetify.display.smAndUp ? '550' : '100%'"
    ...
    <PerfectScrollbar :options="{ wheelPropagation: false, suppressScrollX: true }" class="h-100">
      <VCard>
        <VCardText class="flex d-flex align-center justify-space-between pa-4">
          ...
          <VBtn icon variant="text" color="default" size="x-small" @click="props.toggleModal">
        ...
        <VCardText>
          <VForm ref="refForm" v-model="isFormValid" @submit.prevent="() => { }">
            <VCol class="mb-2" cols="12">
              ...
              <VTextField v-model="editingRole" label="Role Name" type="text" readonly />
            </VCol>
            <VCol class="mb-12" cols="12">
              <p class="text-sm text-capitalize mb-4">
                Role Permissions
              </p>
              <div v-for="(role, i) in props.roles" :key="role.name" class="">
                <p class="mb-0 whitespace-no-wrap">
                  {{ role.name }}
                </p>

                <div v-for="(permission, j) in role.permissions" :key="permission.id" class="d-flex align-center gap-4">
                  <VCol class="pa-0">
                    <VLabel class="text-capitalize">
                      {{ `${permission.codename} ${avatarText(role.name)}` }}
                    </VLabel>
                  </VCol>
                  <VCol class="pa-0">
                    <VCheckbox :value="permission.id" @change="props.handleChange(role, permission, i, j)" />
                  </VCol>
                </div>
              </div>
            </VCol>
