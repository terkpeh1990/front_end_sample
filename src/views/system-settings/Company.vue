...
const props = defineProps({
  ...
  companyData: { type: Array, default: () => [] },
  companyMetaData: { type: Object, default: () => {} },
...
const meta = computed(() => props.companyMetaData)
...
<template>
  <Table
    ...
    :meta-data="meta"
    :is-loading="loading"
    :route="{ name: 'system-settings-company-id' }"
    @page="fetchPageData"
  >
    ...
    <template #cell-status="{ row: company }">
      <VChip
        label
        :color="resolveDataStatusVariant(company.status).variant"
      >
      ...
      >
        ...
        <VMenu activator="parent">
          <VList>
            ...
            <VListItem
              v-if="!company.status"
              @click="$emit('deactivateCompany', {id: company.id, status: true })"
            >
              <template #prepend>
                <VIcon
                  size="24"
            ...
            <VListItem
              v-if="company.status"
              @click="$emit('deactivateCompany', {id: company.id, status: false })"
            >
              <template #prepend>
                <VIcon
                  size="24"
