...
const fetchStockFormFields = async () => {
  fetching.value = true;

  const res = await materialStore.fetchStockForm()
  ...
  if (res.data.data.length !== 0) {
    ...
    const parsedObj = JSON.parse(res.data.data[0].restockform_data.formFields)
...
watchEffect(fetchStockFormFields)

const { loading,  sendForm } = useFormComposable({ type: 'stock', formId: formId, fields: createdFields, editing: isEditing })
