...
export const useChatStore = defineStore('chat', {
  ...
  actions: {
    async fetchChatsAndContacts(q) {
      const { data } = await createAxiosIns().get('/apps/chat/chats-and-contacts', {
        params: { q },
      })
    ...
    async getChat(userId) {
      const { data } = await createAxiosIns().get(`/apps/chat/chats/${userId}`)

      this.activeChat = data
    },
    async sendMsg(message) {
      const senderId = this.profileUser?.id
      const { data } = await createAxiosIns().post(`/apps/chat/chats/${this.activeChat?.contact.id}`, { message, senderId })
      const { msg, chat } = data
